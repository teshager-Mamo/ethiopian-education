// src/components/charts/HighSchoolCharts.js
...
const HighSchoolCharts = ({ filteredData, theme }) => {
  ...
  const getSubjectBarChartData = () => {
    ...
    const classes = [...new Set(filteredData.map((student) => student.predictionclass || 'Unknown'))];
    const subjects = ['english', 'math', 'civic'];
    ...
    const datasets = subjects.map((subject, index) => {
      ...
      return {
        ...
        data: averages,
        backgroundColor: [theme.palette.primary.main, theme.palette.secondary.main, theme.palette.chartAccent.main][index],
        borderColor: [theme.palette.primary.dark, theme.palette.secondary.dark, theme.palette.chartAccent.dark][index],
        borderWidth: 1,
      };
    });
  ...
  const getAgeLineChartData = () => {
    ...
    const ageGroups = [...new Set(filteredData.map((student) => student.age || 'Unknown'))].sort();
      ...
      <Grid item xs={12} md={6}>
        <Paper elevation={4} sx={{ p: 2, borderRadius: 8, background: 'rgba(255, 255, 255, 0.98)' }}>
          ...
          <Typography variant="body2" sx={{ mt: 1, color: '#212121' }}>
            This chart compares average scores in English, Math, and Civic across performance categories.
          </Typography>
        </Paper>
      </Grid>
